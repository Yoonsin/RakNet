...
void CSceneNodeAnimatorCameraFPS::animateNode(ISceneNode* node, u32 timeMs)
{
	...
	if (isPassAnimateNode) {
		isPassAnimateNode = false;
...
void CSceneNodeAnimatorCameraFPS::animateNodeOrigin(ISceneNode* node, u32 timeMs)
{
	//if (!node || node->getType() != ESNT_CAMERA)
	//	return;
	...
	//// get time
	//TickDiff = (f32)(timeMs - LastAnimationTime);
	//LastAnimationTime = timeMs;
	...
	//if (CursorKeys[EKA_MOVE_FORWARD])
	//	pos += movedir * TickDiff * MoveSpeed;
	...
	//if (CursorKeys[EKA_STRAFE_LEFT])
	//	pos += strafevect * TickDiff * MoveSpeed;
