...
int handle_ingress(struct __sk_buff* skb) {
    ...
    if (val) {
        ...
        struct statvalue initval = { .packets = 1, .bytes = skb->len, .ts_init = bpf_ktime_get_ns(), .ts_last = bpf_ktime_get_ns(), .duration = 0 , .avgBytes = skb->len };
...
int handle_egress(struct __sk_buff* skb) {
    ...
    if (val) {
        // atomic XADD, doesn't need bpf_spin_lock() / __sync_fetch_and_add(&val->packets, 1);
        u64 cur_time = bpf_ktime_get_ns();
        bpf_spin_lock(&val->lock);
