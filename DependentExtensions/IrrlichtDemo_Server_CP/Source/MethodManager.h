...
enum ScenarioNum {
	SCENARIO_NONE,			//봇 없음 / 자유로운 행동 가능
	SCENARIO_RESPAWN_BOT,   // 같은 장소에서 리스폰을 끊임없이 반복하는 봇 / 플레이어 사격 불가 
	SCENARIO_MOVE_BOT_RANDOM,      // 랜덤한 방향/속도 이동 -> 리스폰 반복하는 봇 /플레이어 사격 불가
	SCENARIO_MOVE_BOT_FIXED,        // 고정된 방향/속도 이동 (왔다갔다만) -> 리스폰 반복하는 봇 / 플레이어 사격 불가
	SCENARIO_STAND_BOT      // 가만히 서있는 봇 (사격시에만 리스폰) / 플레이어 사격 가능 
};
